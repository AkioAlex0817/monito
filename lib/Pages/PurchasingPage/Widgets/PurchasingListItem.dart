...
class PurchasingListItem extends StatelessWidget {
  final PurchasingModel purchasingModel;
  final VoidCallback onPressed;

  PurchasingListItem({@required this.purchasingModel, @required this.onPressed});
              ...
              children: [
                Container(
                  ...
                  child: AspectRatio(
                    aspectRatio: 1,
                    child: Hero(
                      tag: 'purchasing_image_' + purchasingModel.asin,
                      child: CachedNetworkImage(
                        placeholder: (context, url) => CupertinoActivityIndicator(radius: 10),
                        errorWidget: (context, url, error) => Icon(Icons.error, color: Colors.red),
                        imageUrl: Helper.imageURL(purchasingModel.photo),
                      ),
                    ),
                  ),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Hero(
                        tag: "purchasing_title_" + purchasingModel.asin,
                        child: Container(
                      ...
                      Row(
                        ...
                        children: [
                          Text(
                            sprintf("%s位", [purchasingModel.sales_rank.formatter]),
                            style: TextStyle(color: Colors.red, fontSize: 12, fontWeight: FontWeight.bold),
                          ),
                          Text(
                            sprintf("%s円", [purchasingModel.cart_price == -1 ? purchasingModel.new_price.formatter : purchasingModel.cart_price.formatter]),
                            style: TextStyle(color: Colors.red, fontSize: 12, fontWeight: FontWeight.bold),
                          )
                        ],
                      ),
                      Container(
                        ...
                        child: Text(Helper.formatDate(DateTime.parse(purchasingModel.created_at), 'yyyy-MM-dd'), style: TextStyle(color: Colors.black, fontSize: 12, fontWeight: FontWeight.bold)),
                      )
                    ],
                  ),
                )
